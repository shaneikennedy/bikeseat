...
fn main() {
    let markdown: String = fs::read_to_string("index.md").expect("could not read file.");
    let html_str = Parser::parse_md(markdown.lines().map(|l| l.trim()).collect());
    let template = fs::read_to_string("templates/index.template").expect("could not find template file");
